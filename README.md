...
- `prices.json.gz` (~12MB) จะถูก commit ทับทุกวันที่มีข้อมูลใหม่ ทำให้**ขนาด repo โตขึ้นเรื่อยๆ ตามจำนวนวัน** (ปีละหลาย GB ถ้านับรวมประวัติ commit) ถ้าเริ่มใหญ่เกินไป แก้ได้ด้วยการ squash ประวัติ commit เป็นระยะ (`git checkout --orphan`) หรือย้ายไปใช้ Git LFS
