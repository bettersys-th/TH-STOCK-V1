...
def swing_signal_at(ticker, dates, close, volume, idx=None):
    """Return a Swing signal using bars at or before idx only."""
    idx = len(close) - 1 if idx is None else idx
    if idx < 204:
        return None
    p, v = close[:idx + 1], volume[:idx + 1]
    current = p[-1]
    ma20, ma50, ma200 = _mean(p[-20:]), _mean(p[-50:]), _mean(p[-200:])
    prior_high20, prior_high60 = max(p[-21:-1]), max(p[-61:-1])
    low20 = min(p[-20:])
    avg_vol20, avg_vol60 = _mean(v[-20:]), _mean(v[-60:])
    vol_ratio = avg_vol20 / avg_vol60 if avg_vol60 else 0
    value20 = median([price * vol for price, vol in zip(p[-20:], v[-20:])])
    momentum20 = current / p[-21] - 1
    momentum60 = current / p[-61] - 1
    distance_breakout = current / prior_high20 - 1
    stop = low20
    risk = current - stop
    historical_target = max(prior_high60, max(p[-252:]))
    ...
    date_text = str(dates[idx])
    return {
        "t": ticker, "date": f"{date_text[:4]}-{date_text[4:6]}-{date_text[6:8]}",
...
def build_swing_signals(prices, splits, tickers, active_window_days=60):
    ...
    for ticker in tickers:
        ...
        signal = swing_signal_at(ticker, dates, close, volume)
        if signal:
            rows.append(signal)
