...
const {SCENARIO_PRESETS,boardLotPurchase,simulateScenario,contributionPerPeriod,assessMonthlyBudget,rankAffordableAlternatives,analyzeOutcomeTimeline}=window.DcaDomain;
...
const PLAN_FIELDS=['riskInitial','riskBudget','riskFrequency','riskMonths','riskBuyMonths','riskDecline','riskDeclineMonths','riskUseCycleTime','riskTarget','riskTolerance','riskDividend'];
...
function calculateRisk(){
 ...
 const ledger=[];
 if(initial>0)ledger.push({step:0,period:'เริ่มต้น',phase:firstPurchase.shares?'ราคาปัจจุบัน':'รอเงินครบ 100 หุ้น',price:current,buy:firstPurchase.cost,bought:firstPurchase.shares,shares,invested,value:shares*current,pnl:shares*current-invested,pct:0});
 for(let i=1;i<=steps;i++){
  ...
  ledger.push({step:i,period:`${i}/${steps}`,phase:i>buySteps?'หยุดซื้อและติดตามพอร์ต':bought?(i<=downSteps?'ราคาลง':'ราคาฟื้น'):'รอเงินครบ 100 หุ้น',price,buy,bought,shares,invested,value,pnl,pct});
 }
