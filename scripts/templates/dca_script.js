...
const {SCENARIO_PRESETS,boardLotPurchase,simulateScenario,contributionPerPeriod,assessMonthlyBudget,rankAffordableAlternatives,analyzeOutcomeTimeline,buildPricePath}=window.DcaDomain;
...
const PLAN_FIELDS=['riskInitial','riskBudget','riskFrequency','riskMonths','riskBuyMonths','riskPathModel','riskDecline','riskDeclineMonths','riskUseCycleTime','riskTarget','riskTolerance','riskDividend'];
...
function calculateRisk(){
 ...
 for(let i=1;i<=steps;i++){
  ...
  ledger.push({step:i,period:`${i}/${steps}`,phase:i>buySteps?'หยุดซื้อและติดตามพอร์ต':bought?(i<=downSteps?'ราคาลง':'ราคาฟื้น'):'รอเงินครบ 100 หุ้น',price,buy,bought,shares,invested,reserve,value,pnl,pct});
 }
 ...
 const outcome=analyzeOutcomeTimeline({points:ledger.map(row=>({period:row.step,price:row.price,invested:row.invested,value:row.value})),periodsPerMonth,buyPeriods:buySteps,startPrice:current,declineEndPeriod:downSteps}),monthText=value=>value===null?`ยังไม่เกิดใน ${months} เดือน`:value===0?'ทันที':`เดือนที่ ${value}`;
 ...
 renderPortfolioChart(ledger,current,budget,months);document.getElementById('pathModelNote').textContent=pathResult.fallbackReason?'ข้อมูลรายวัน 3 เดือนยังไม่มีใน Cloud ชุดนี้ จึงใช้เส้นตรงแทน':pathResult.modelUsed==='recent3m'?`ใช้ราคาปิดย้อนหลัง ${pathResult.sourcePoints} วัน ปรับขนาดตาม Scenario`:pathResult.modelUsed==='cycle'?'ใช้เส้นโค้ง Smooth Cycle ตามระยะขาลงและการฟื้นตัว':pathResult.modelUsed==='stress'?'ใช้เส้นทางผันผวนแบบ deterministic เพื่อให้ตรวจซ้ำได้':'ใช้การลดและฟื้นแบบคงที่สำหรับตรวจสูตร';
